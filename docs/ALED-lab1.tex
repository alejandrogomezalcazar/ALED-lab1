...
\begin{enumerate}
    ...
    \item Clone su \textit{fork} e impórtelo en Eclipse. Para ello, seleccione \textit{File $\to$ Import...} y, de entre las opciones disponibles, \textit{Git $\to$ Projects from Git $\to$ Clone URI}. A continuación, introduzca la URI de su \textit{fork} (\texttt{https://github.com/}\textit{usuario}\texttt{/ALED-lab1}, donde \textit{usuario} es su nombre de usuario de GitHub), no la del repositorio original. ¡Ojo, la captura de pantalla que verá más adelante muestra la URL del repositorio original, no la suya (lógicamente)! Después, haga click en \textit{Next} tantas veces como sea necesario y, por último, en \textit{Finish}.
